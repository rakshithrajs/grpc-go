...
func (h *UserHandler) UpdateUserHandler(c *gin.Context) {
	...
	if err != nil {
		handlerUtils.ReturnErrorResponse(c, err, FnUpdateUserProfile, middlewareUtils.ErrSomethingWentWrong, config.NullString)
		return
	}
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		handlerUtils.ReturnErrorResponse(c, handlerUtils.ErrInvalidJSON, FnUpdateUserProfile, middlewareUtils.ErrSomethingWentWrong, config.NullString)
		return
	}

	if req.Name == config.NullString && req.Email == config.NullString && req.Phone == config.NullString && req.Password == config.NullString {
		handlerUtils.ReturnErrorResponse(c, handlerUtils.ErrNoFieldsToUpdate, FnUpdateUserProfile, middlewareUtils.ErrSomethingWentWrong, config.NullString)
		return
	}

	if err := modelUtils.Validate.Struct(req); err != nil {
		handlerUtils.ReturnErrorResponse(c, modelUtils.FieldErrors(err), FnUpdateUserProfile, middlewareUtils.ErrSomethingWentWrong, config.NullString)
		return
	}
	...
	if req.Password != config.NullString {
		...
		if err := bcrypt.CompareHashAndPassword([]byte(user.Password), []byte(req.Password)); err == nil {
			handlerUtils.ReturnErrorResponse(c, handlerUtils.ErrPasswordSameAsOldPassword, FnUpdateUserProfile, middlewareUtils.ErrSomethingWentWrong, config.NullString)
			return
		}
