...
func main() {
	...
	flag.StringVar(&token, "token", nullString, "JWT token for authentication (or MMS_TOKEN env)")
	flag.StringVar(&MMSAddr, "addr", cfg.GRPCAddress, "MMS gRPC server address")
	flag.Parse()

	if token == nullString {
		fmt.Printf("missing authentication token: provide --token flag or set MMS_TOKEN env var\n")
		os.Exit(1)
	}

	conn, err := grpc.NewClient(MMSAddr, grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
	...
	MMSClient = MMSpb.NewFilesClient(conn)
...
func runInteractive() error {
	for {
		fmt.Println()
		fmt.Println("MMS service client")
		fmt.Println("1. Upload a file")
		fmt.Println("2. List MMS")
...
func promptAndUploadFile() error {
	...
	resp, err := MMSClient.UploadFile(ctx, &MMSpb.UploadFileRequest{
...
func runListMMS() error {
	ctx, cancel := withTimeout()
	defer cancel()
	ctx = withAuth(ctx)

	resp, err := MMSClient.ListFiles(ctx, &MMSpb.ListFilesRequest{})
	if err != nil {
		return err
	}

	if len(resp.GetFile()) == 0 {
		fmt.Println("no MMS found")
		return nil
	}
...
func promptAndDownloadFile() error {
	...
	resp, err := MMSClient.DownloadFile(ctx, &MMSpb.DownloadFileRequest{FileID: fileID})
	if err != nil {
		return err
	}
...
func promptAndRenameFile() error {
	...
	_, err := MMSClient.RenameFile(ctx, &MMSpb.RenameFileRequest{
		FileID:  fileID,
		NewName: newName,
	})
	if err != nil {
...
func promptAndDeleteFile() error {
	...
	_, err := MMSClient.DeleteFile(ctx, &MMSpb.DeleteFileRequest{FileID: fileID})
	if err != nil {
		return err
	}
